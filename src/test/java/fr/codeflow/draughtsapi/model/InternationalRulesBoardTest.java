...
import fr.codeflow.draughtsapi.services.BoardFactory;
...
class InternationalRulesBoardTest {
    ...
    public void Should_Have50ActiveSquare_When_InitIsOK() {
        ...
        var internationalRulesBoard = new BoardFactory().createSquare();
        ...
        for (int rowIndex = 0; rowIndex < Arrays.stream(internationalRulesBoard).count(); rowIndex++) {
            for (int columnIndex = 0; columnIndex < internationalRulesBoard[rowIndex].length; columnIndex++) {
                var currentSquare = internationalRulesBoard[rowIndex][columnIndex];
